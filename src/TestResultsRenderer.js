const TestResultsRenderer = class {
	...
	_listenToRunner() {
		this._runner.addListener((...params) => {
			this._handleRunStarted(...params)
		}, this._runner.events.RunStarted)

		this._runner.addListener((...params) => {
			this._handleRunEnded(...params)
		}, this._runner.events.RunEnded)

		this._runner.addListener((...params) => {
			this._handleTestStarted(...params)
		}, this._runner.events.Started)

		this._runner.addListener((...params) => {
			this._handleSetupSucceeded(...params)
		}, this._runner.events.SetupSucceeded)

		this._runner.addListener((...params) => {
			this._handleSetupFailed(...params)
		}, this._runner.events.SetupFailed)

		this._runner.addListener((...params) => {
			this._handleTestPassed(...params)
		}, this._runner.events.Passed)

		this._runner.addListener((...params) => {
			this._handleTestFailed(...params)
		}, this._runner.events.Failed)

		this._runner.addListener((...params) => {
			this._handleTeardownSucceeded(...params)
		}, this._runner.events.TeardownSucceeded)

		this._runner.addListener((...params) => {
			this._handleTeardownFailed(...params)
		}, this._runner.events.TeardownFailed)
	}
}
